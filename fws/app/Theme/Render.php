...
class Render extends Singleton
{
    ...
    public function getPostedOn(string $format = ''): string
    {
        ...
        $authorPageLink = get_author_posts_url(intval(get_the_author_meta('ID')));

        return '<div class="entry-meta"><span class="posted-on">Posted on <a href="' . $link . '" class="post_url"><span>'
            . $date . '</span></a> by <a class="author_name" href="' . $authorPageLink . '">' . $author . '</a></span></div>';
    }
    ...
    public function pagingNav(): void
    {
        ...
        // Set up paginated links, // there is phpstan errors bcs of improper "paginate_links" declaration
    ...
    public function pageDefaultHeader(string $title, string $subtitle = '', bool $isScreenReader = false): void
    {
        ...
        echo '<h1 class="page-title' . ($isScreenReader ? ' screen-reader-text">' : '">') . esc_html($title) . '</h1>';
        if ($subtitle) {
            echo '<div class="archive-description">' . esc_html($subtitle) . '</div>';
        }
     ...
     * @return void
     */
    public function varDump($value): void // phpcs:ignore Inpsyde.CodeQuality.ArgumentTypeDeclaration
    {
        ...
        var_dump($value); // phpcs:ignore WordPress.PHP.DevelopmentFunctions
        echo '</pre>';
    }
    ...
    {
        return '<span style="display: block; color: white; font-weight: bold; background-color: red; padding: 10px; text-align: center;">'
            . esc_html($message) . ':<br><span style="font-weight: normal">' . esc_html($path) . '</span></span>';
    }


    /**
     * Return "nonce" that will be used in "wp_localize_script()" and validation of AJAX requests.
     *
     * @return string
     */
    public function localizedNonceAction(): string
    {
        return md5('fwsNonce/' . sanitize_text_field($_SERVER['HTTP_HOST'] ?? ''));
    }
