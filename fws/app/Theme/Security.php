...
class Security
{
    ...
    public static function isSuperAdmin(): bool
    {
        ...
        return $user->user_email && in_array($user->user_email, fws()->fwsConfig()->superadminEmails(), true);
    }
    ...
    public static function isLocalEnvironment(): bool
    {
        ...
        if ($cached === null) {
            $cached = false;
            $home = home_url();
            foreach (fws()->fwsConfig()->allowedLocalhosts() as $host) {
    ...
    public static function preventPluginsUpdateHook(): void
    {
        ...
        if (!fws()->fwsConfig()->pluginsOnlyLocalEditing() || self::isSuperAdmin() || self::isLocalEnvironment()) {
            return;
        }
    ...
    {
        if (fws()->fwsConfig()->clickjackingProtection()) {
