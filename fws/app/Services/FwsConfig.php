 ...
 * Class FwsConfig
 */
class FwsConfig extends Singleton
{
    ...
    {
        return strval($this->config['global']['theme-name'] ?? '');
    }
    ...
    {
        return boolval($this->config['acf-options-page']['enable'] ?? true);
    }
    ...
    {
        return strval($this->enqVersion['enqueue-version'] ?? '');
    }
    ...
    {
        return strval($this->config['global']['cf7-custom-templates'] ?? '');
    }
     ...
     * @return FlexContent
     */
    private function mapFlexContent(array $args): FlexContent
    {
